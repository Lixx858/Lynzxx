...
export default function Settings(){
 const[k,setK]=useState(""),[show,setShow]=useState(false),[msg,setMsg]=useState(""),[saved,setSaved]=useState(false),[model,setModel]=useState("gemini-3.6-flash"),[worker,setWorker]=useState(false);
 useEffect(()=>{try{const x=sessionStorage.getItem("clipforge_gemini_key")||"";setK(x);setSaved(!!x);setModel(sessionStorage.getItem("clipforge_gemini_model")||"gemini-3.6-flash")}catch{}fetch("/api/settings").then(r=>r.json()).then(x=>setWorker(x.workerConfigured))},[]);
 ...
 <label>Model</label><select value={model} onChange={e=>setModel(e.target.value)}><option value="gemini-3.6-flash">Gemini 3.6 Flash</option><option value="gemini-3.5-flash">Gemini 3.5 Flash</option><option value="gemini-3.1-flash">Gemini 3.1 Flash</option><option value="gemini-3.1-pro-preview">Gemini 3.1 Pro (Preview)</option></select>
